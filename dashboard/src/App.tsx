...
import { Activity, Bell, ChevronDown, ChevronRight, CircleDot, Fuel, Gauge, MapPin, Radio, RefreshCw, Search, ShieldAlert, Thermometer, Truck, Wifi, WifiOff, X } from 'lucide-react'
...
function AlertsPanel({ alerts, expanded, onToggle, onSelect }: { alerts: FleetAlertResponse[]; expanded: boolean; onToggle: () => void; onSelect: (alert: FleetAlertResponse) => void }) { const visibleAlerts = expanded ? alerts : alerts.slice(0, 4); return <section className="panel alerts-panel"><div className="panel-heading"><div><span className="panel-kicker">ATTENTION QUEUE</span><h2>Recent alerts <span className="heading-count">{alerts.length}</span></h2></div><button className="text-button" onClick={onToggle}>{expanded ? 'Collapse' : 'View all'} <ChevronRight size={14} /></button></div><div className="alert-list">{visibleAlerts.length === 0 ? <div className="empty-state compact-empty"><ShieldAlert size={20} /><p>No active threshold alerts.</p></div> : visibleAlerts.map((alert) => <button className="alert-row" key={alert.id} onClick={() => onSelect(alert)}><div className={`alert-icon ${alert.severity.toLowerCase()}`}><ShieldAlert size={15} /></div><div className="alert-copy"><strong>{alert.vehicleExternalId}</strong><span>{alert.message}</span><small>{formatAlertTime(alert.triggeredAt)}</small></div><ChevronRight className="alert-chevron" size={15} /></button>)}</div><button className="panel-footer-button" onClick={onToggle}>{expanded ? 'Close alert center' : 'Open alert center'} <ChevronRight size={14} /></button></section> }
